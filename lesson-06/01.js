...
const imageSlide = document.getElementById('web-tech-image');
let indexImage = 0;

function newImage() {
  imageSlide.src = WEB_TECH_IMAGES[indexImage];
  
}
const buttonImagePrev = document.getElementById('prev-button');
buttonImagePrev.addEventListener('click', function() {
  indexImage = (indexImage - 1 + WEB_TECH_IMAGES.length) % WEB_TECH_IMAGES.length
  newImage();
})

const buttonImageNext = document.getElementById('next-button');
buttonImageNext.addEventListener ('click', function() {
  indexImage = (indexImage + 1) % WEB_TECH_IMAGES.length
  newImage();
})